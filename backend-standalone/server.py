...
def get_rover_data():
    ...
    print(f"[DEBUG] get_rover_data() sending rtk_fix_type={current_state.rtk_fix_type}, rtk_base_linked={current_state.rtk_base_linked}", flush=True)
    ...
    try:
        if msg_type == "state":
            ...
            with mavros_telem_lock:
                ...
                # Extract baseline age if available from message
                # Note: MAVROS gps_fix may not always have baseline_age in the message
                # This would typically come from a separate RTK-specific topic
